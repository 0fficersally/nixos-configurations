...
{ config, lib, pkgs, ... }: {
  options = {
    modules.environments.desktop.swaynotificationcenter.enable = lib.mkEnableOption "the SwayNotificationCenter notification daemon";
  };

  config = lib.mkIf config.modules.environments.desktop.swaynotificationcenter.enable {
    services.swaync = {
      enable = true;
